...
class AllThreMseLoss(nn.Module):
    ...
    def forward(self, preds, targets, lengths: Optional[Union[Tensor, List[int]]], threshold):
        if isinstance(lengths, torch.Tensor):
            lengths: List[int] = lengths.tolist()

        # preds: [bsz * k, 25, n_frames] <-- [bsz, 25, n_frames]
...
class MidLoss(nn.Module):
    ...
    def forward(self, inputs, lengths: Optional[Union[Tensor, List[int]]]):
        if isinstance(lengths, torch.Tensor):
            lengths: List[int] = lengths.tolist()
